...
module.exports.run = async (
	...
	if (args.length === 0) {
		let ghrt = new Discord.MessageEmbed()
			...
			.setTitle("AP:Remove")
			.setDescription(
				"Remove AP from an existing user's AP. You can mention the user or use their ID with this command!"
			)
			.addField("Initiation:", "%ap:remove")
			.addField(
				"Correct Usage:",
				"%ap:remove [user(ID)] <AP>\n\nUser === GuildMemberProperty\nUserID === GuildMemberProperty\nAP: Number"
			)
			.addField("Example:", `%ap:remove @${message.author.tag} 1`)
			.addField("Permissions I need:", "Manage Roles, Manage Messages")
			.addField("Permissions you need:", "Manage Roles")
			.addField("Aliases:", '["apremove", "remove"]')
			.setFooter("Stable");
	...
	const start = async () => {
		...
		async function accessSpreadsheet() {
			...
			//[SPR1 + SPR2] Switch Case Break function that determines which rank to give depending on Attendance Points.
			...
			if (Whitelist == false) {
				...
				if (!toAP.roles.cache.has(ranktogive[0])) {
					...
					await toAP.roles.add(ranktogive).catch((err) => {
						message.channel.send(
							`Somehow, ${toAP} got an unknown rank in the code!`
						);
					...
					await message.channel.send(
						`${toAP} has been \`demoted to\` ${row4.rankname}!`
					);
				}
			} else {
	...
	start().then(() => {
		...
		let aws = new Discord.MessageEmbed() //Determine what type of message discord should send.
			...
			.addField("Action:", `[\`Removed\`](${video()})`)
			.addField("Amount:", `[\`${args[args.length - 1]}\`](${video()})`)
			.addField("From:", `[\`${toAP.user.tag}\`](${video()})`)
