...
module.exports.run = async (
	...
	if (args.length === 0) {
		let ghrt = new Discord.MessageEmbed()
			...
			.setTitle("Vc:remove")
			.setDescription(
				"Remove AP from an existing array of users in a Voice Chat. You must be in the voice channel of where you want ap to be removed from."
			)
			.addField("Initiation:", "%vc:remove")
			.addField(
				"Correct Usage:",
				"%vc:remove <AP>\n\nUsers === GuildMemberProperty\nAP: Number"
			)
			.addField("Example:", `%vc:remove 1`)
			.addField("Permissions I need:", "Manage Roles, Manage Messages")
			.addField("Permissions you need:", "Manage Roles")
			.addField("Aliases:", '["vcremove", "massremove"]')
			.setFooter("Stable");

		DBTrue();
		return message.channel.send({ embed: ghrt });
	}
	...
	start().then(() => {
		...
		let aws = new Discord.MessageEmbed() //Determine what type of message discord should send.
			...
			.addField("Action:", `[\`Mass Removed\`](${video()})`)
			.addField("Amount:", `[\`${args[args.length - 1]}\`](${video()})`)
			.addField("From Users:", `${myMap}`)
			.setFooter(
				`Time: ${
