...
module.exports.run = async (bot, message, args, myVideos, creds) => {
	...
	if (args.length === 0) {
		let ghrt = new Discord.MessageEmbed()
			...
			.setTitle("AP:Recruitedremove")
			.setDescription(
				"Remove recruitment points from a user. You can mention the user or use their ID with this command!"
			)
			.addField("Initiation:", "%ap:recruitedremove")
			.addField(
				"Correct Usage:",
				"%ap:recruitedremove [user(ID)] <RP>\n\nUser === GuildMemberProperty\nUserID === GuildMemberProperty\nRP: Number"
			)
			.addField("Example:", `%ap:recruitedremove @${message.author.tag} 1`)
			.addField("Permissions I need:", "Manage Messages")
			.addField("Permissions you need:", "Manage Roles")
			.addField("Aliases:", '["recruited:remove", "r:remove", "rremove"]')
			.setFooter("Stable");
...
module.exports.help = {
	name: "recruitedremove",
	aliases: ["recruited:remove", "r:remove", "rremove"],
};
