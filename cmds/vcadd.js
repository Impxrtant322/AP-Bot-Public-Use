...
module.exports.run = async (
	...
	if (args.length === 0) {
		let ghrt = new Discord.MessageEmbed()
			...
			.setTitle("Vc:add")
			.setDescription(
				"Add AP to an existing array of users in a Voice Chat. You must be in the voice channel of where you want ap to be added to."
			)
			.addField("Initiation:", "%vc:add")
			.addField(
				"Correct Usage:",
				"%vc:add <AP>\n\nUsers === GuildMemberProperty\nAP: Number"
			)
			.addField("Example:", `%vc:add 1`)
			.addField("Permissions I need:", "Manage Roles, Manage Messages")
			.addField("Permissions you need:", "Manage Roles")
			.addField("Aliases:", '["vcadd", "massadd"]')
			.setFooter("Stable");
