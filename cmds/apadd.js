...
module.exports.run = async (
	...
	if (args.length === 0) {
		let ghrt = new Discord.MessageEmbed()
			...
			.setTitle("AP:Add")
			.setDescription(
				"Add AP to an existing user's AP. You can mention the user or use their ID with this command!"
			)
			.addField("Initiation:", "%ap:add")
			.addField(
				"Correct Usage:",
				"%ap:add [user(ID)] <AP>\n\nUser === GuildMemberProperty\nUserID === GuildMemberProperty\nAP: Number"
			)
			.addField("Example:", `%ap:add @${message.author.tag} 1`)
			.addField("Permissions I need:", "Manage Roles, Manage Messages")
			.addField("Permissions you need:", "Manage Roles")
			.addField("Aliases:", '["apadd", "add"]')
			.setFooter("Stable");
